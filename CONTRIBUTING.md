...
Thanks for helping Improve Applykit. This is an MIT-licensed side project. Keep it small, honest, and safe.
...
- **Privacy by default.** Candidate data stays in `APPLYKIT_HOME` or `~/.applykit/`. Do not log résumés, emails, or phone numbers. Telemetry stays off unless the user enables it, and then only the documented fields may be previewed.
...
- Put fake résumés in `applykit/fixtures/resumes/`.
